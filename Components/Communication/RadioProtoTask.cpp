...
void RadioProtocolTask::HandleProtobufCommandMessage(EmbeddedProto::ReadBufferFixedSize<PROTOCOL_RX_BUFFER_SZ_BYTES>& readBuffer)
{
    ...
    case Proto::FcbCommand::Command::RSC_AIR_BRAKES_DISABLE: {
    	RPB_AIR_BRAKES_COMMAND cmd;
    	cmd.openAirBrakes = false;
    	SOAR_PRINT("airbrakes disable\n");
    	CANTask::Inst().SendCANMessageToDaughter(CAN_ROCKET_TARGET_RPB, _RPB_AIR_BRAKES_COMMAND_LOGINDEX, (uint8_t*)&cmd);
		break;
    }

    case Proto::FcbCommand::Command::RSC_AIR_BRAKES_ENABLE: {
    	RPB_AIR_BRAKES_COMMAND cmd;
