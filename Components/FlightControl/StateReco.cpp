...
StateRecoverer::StateSave StateRecoverer::GetSave(RecoSectorIndex index) {
	...
	StateSave s;
	if(index >= sectorCount || !SPIFlash::Inst().Read((sectorStart+index)*SPIFlash::Inst().GetSectorSize(), (uint8_t*)&s, sizeof(s))) {
...
bool StateRecoverer::PutSave(RecoSectorIndex index, StateSave state) {
	if(index >= sectorCount || !SPIFlash::Inst().Erase((sectorStart+index)*SPIFlash::Inst().GetSectorSize())) {
		return false;
	}

	return SPIFlash::Inst().Write((sectorStart+index)*SPIFlash::Inst().GetSectorSize(),
			(uint8_t*)&state,
			sizeof(state));
}
